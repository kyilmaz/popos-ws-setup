...
# if ! is_installed cuda-toolkit-12-2; then
    ...
    # sudo apt-key del 7fa2af80 || log_warning "Failed to delete old CUDA keyring"
...
if ! command -v conda &>/dev/null; then
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        wget -q "$DOWNLOAD_URL" -O "$INSTALL_PATH" || log_warning "Failed to download Miniconda"

        log "Installing Miniconda..."
        bash "$INSTALL_PATH" -b -p "$INSTALL_DIR" || log_warning "Failed to install Miniconda"

        log "Initializing Conda..."
        "$INSTALL_DIR/bin/conda" init bash || log_warning "Failed to initialize conda"
        rm -f "$INSTALL_PATH"
